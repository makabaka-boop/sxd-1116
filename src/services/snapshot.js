...
function buildCurrentRealState() {
  const today = new Date().toISOString().substring(0, 10);
  const activeScheduleRules = store.scheduleRules.filter((r) => {
    if (r.effective_date && r.effective_date > today) return false;
    if (r.expired_date && r.expired_date < today) return false;
    return true;
  });
  ...
  const maintenancePlans = getMaintenancePlansForDate(today);
  const maintenanceDeskIds = new Set(maintenancePlans.map((p) => p.desk_id));
...
function compareSnapshotWithCurrent(dateStr) {
  ...
  const current = buildCurrentRealState();
